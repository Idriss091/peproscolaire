<template>
  <div class="space-y-6">
    <form @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div v-if="competence.evaluations_count > 0" class="bg-gray-50 p-4 rounded-lg">
        <h4 class="text-sm font-medium text-gray-900 mb-2">Statistiques d'utilisation</h4>
        <div class="grid grid-cols-2 gap-4 text-sm text-gray-600">
          <div>
            <span class="font-medium">Évaluations :</span> {{ competence.evaluations_count }}
          </div>
          <div>
            <span class="font-medium">Élèves évalués :</span> {{ competence.students_count }}
          </div>
          <div>
            <span class="font-medium">Maîtrise moyenne :</span> {{ competence.average_mastery }}%
          </div>
        </div>
      </div>
...
const initializeForm = () => {
  if (props.competence) {
    form.code = props.competence.code || ''
    form.description = props.competence.description || ''
    form.subject_id = props.competence.subject_id || ''
    form.level = props.competence.level || ''
    form.domain = props.competence.domain || ''
    form.criteria = props.competence.criteria || ['']
    form.status = props.competence.status || 'active'
  }
}
...
const handleSubmit = async () => {
  ...
  loading.value = true
  try {
    // TODO: Appel API pour modifier la compétence
    console.log('Modification de la compétence:', form)
